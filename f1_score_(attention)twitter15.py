# -*- coding: utf-8 -*-
"""f1_score_(Attention)twitter15_experiment_2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1jG4QMPOk4g5TVtAzZRKxV-1gvJrcIquG
"""
...
label_dict = np.load('/content/drive/My Drive/twitter15_twitter16_saved_files/twitter15_files/label_dict.npy', allow_pickle=True).item()
data = np.load('/content/drive/My Drive/twitter15_twitter16_saved_files/twitter15_files/data.npy', allow_pickle=True).tolist()
tweets = np.load('/content/drive/My Drive/twitter15_twitter16_saved_files/twitter15_files/tweets.npy', allow_pickle=True).tolist()
twitter_15_text_dict = np.load('/content/drive/My Drive/twitter15_twitter16_saved_files/twitter15_files/twitter_15_text_dict.npy', allow_pickle=True).item()
src_tweets_text = np.load('/content/drive/My Drive/twitter15_twitter16_saved_files/twitter15_files/src_tweets_text.npy', allow_pickle=True).item()

for key, directory in data.items():
    structure = directory['structure']
    structure_text = []
    label = label_dict[key]
    for item in structure:
        try:
            if twitter_15_text_dict[item[1]]['lang'] == 'en':
                structure_text.append([src_tweets_text[item[0]], twitter_15_text_dict[item[1]]['text']])
        except KeyError:
            continue
    data[key]['label_str'] = label
    if label == 'non-rumor':
        label_int = 0
        ...
        label_int = 3
    data[key]['input_text'] = structure_text
    data[key]['label_int'] = label_int


for key in list(data.keys()):
    if len(data[key]['structure']) == 0 or len(data[key]['input_text']) == 0:
        del data[key]

for key, value in data.items():

    if len(data[key]['input_text']) > 100:
        data[key]['input_text'] = value['input_text'][:100]
...
for item in data.values():
  ...
  for sent in data_use:
    ...
    sent.append(encoded_dict['input_ids'])
    sent.append(encoded_dict['attention_mask'])
    sent.append(encoded_dict['token_type_ids'])

data['273278761909239808']['input_text'][0]
...
def collate_fn2(batch):
    ...
    la = []
    token_type = []
    for item in batch[0]['input_text']:
        in_id.append(item[2])
        mask.append(item[3])

        token_type.append(item[4])
    label_int = batch[0]['label_int']
    number = len(batch[0]['input_text'])
    la = [label_int]*number
    return (in_id, mask, torch.tensor(la), token_type)

from torch.utils.data import TensorDataset, random_split
dataset = MyDataset(list(data.values()))
train_size = int(0.85 * len(dataset))
val_size = len(dataset) - train_size
train_dataset, val_dataset = random_split(dataset, [train_size, val_size])
...
for epoch_i in range(0, epochs):
    ...
    print("  macro_F1_score: {}".format(f1score))
